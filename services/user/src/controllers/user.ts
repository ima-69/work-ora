...
export const updateProfilePicture = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    const { data: uploadResult } = await axios.post<{ url: string; public_id: string }>(
...
export const deleteSkillFromUser = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (result.length === 0) {
        throw new ErrorHandler(404, `Skill '${skillName}' not found for the user`);
    }
...
export const applyForJob = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (!job) {
    throw new ErrorHandler(404, "No job found with the provided ID");
  }
